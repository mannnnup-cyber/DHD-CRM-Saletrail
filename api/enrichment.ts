...
// ---------------------------------------------------------------------------
// Shared helpers (one copy, used by all actions)
// ---------------------------------------------------------------------------
...
async function findCompanyWebsite(companyName: string): Promise<string | null> {
  ...
  for (const domain of domains) {
    try {
      ...
      if (result && (result.email || result.phone || result.description)) return domain;
    } catch {
      continue;
    }
  }
...
/**
 * POST /api/enrichment?action=enrichContacts
 * Bulk enrich contacts from company names (from enrichBulk.ts)
 */
async function enrichContacts(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    for (const contact of contacts) {
      ...
      if (!companyName) {
        results.push({ input: contact, success: false, error: 'No company name provided', enriched: null });
        failureCount++;
        continue;
      }
      ...
      try {
        ...
        if (found) {
          ...
          results.push({ input: contact, success: true, enriched: { ...contact, ...found, enrichment_source: 'bulk_web_scrape', enrichment_confidence: confidence, enrichment_timestamp: new Date().toISOString() } });
          successCount++;
        } else {
          results.push({ input: contact, success: false, error: 'Could not find website for company', enriched: null });
          failureCount++;
        }
      } catch (error: any) {
        results.push({ input: contact, success: false, error: error.message || 'Enrichment failed', enriched: null });
        failureCount++;
      }
    ...
    console.error('[enrichment] enrichContacts error:', err);
    return res.status(500).json({ success: false, error: `Server error: ${err.message || 'Unknown error'}` });
  }
}

/**
 * POST /api/enrichment?action=previewEnrichment
 * Preview bulk enrichment (trial run on sample) (from enrichBulk.ts)
 */
async function previewEnrichment(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const projectedSuccessRate = sample.length > 0 ? Math.round((successCount / sample.length) * 100) : 0;
    const projectedSuccessCount = Math.round((projectedSuccessRate / 100) * contacts.length);

    return res.status(200).json({ success: true, preview: { sampleSize: sample.length, totalContacts: contacts.length, sampleSuccessful: successCount, sampleFailed: sample.length - successCount, projectedSuccessRate, projectedSuccessful: projectedSuccessCount, projectedFailed: contacts.length - projectedSuccessCount } });
  } catch (err: any) {
    console.error('[enrichment] previewEnrichment error:', err);
    return res.status(500).json({ success: false, error: `Server error: ${err.message || 'Unknown error'}` });
  }
}

/**
 * POST /api/enrichment?action=enrichLead
 * Enrich a single lead/contact from a company URL (from scrape.ts)
 */
  ...
  try {
    ...
    if (!targetUrl && useCompanyName) {
      ...
      if (contactError || !contact?.company) {
        return res.status(400).json({ success: false, error: 'Contact has no company name to detect website' });
      }
      ...
      if (!targetUrl) {
        return res.status(400).json({ success: false, error: `Could not find website for company: ${contact.company}. Try entering URL manually.` });
      }
    }
    ...
    if (!extractedData) return res.status(500).json({ success: false, error: 'Failed to scrape website. Check URL and try again.' });
    ...
    if (Object.keys(updatePayload).length === 0) return res.status(400).json({ success: false, error: 'No data extracted from website', extracted: extractedData });

    const { data, error } = await supabase.from('contacts').update(updatePayload).eq('id', contactId).select('*').single();
    if (error) {
      console.error('[enrichment] Update error:', error.message);
      return res.status(500).json({ success: false, error: `Database update failed: ${error.message || 'Unknown error'}`, extracted: extractedData, supabaseError: error.message });
    }

    try {
      await supabase.from('interactions').insert({ contact_id: contactId, type: 'ENRICHMENT', subject: `Web scrape enrichment: ${targetUrl}`, content: `Enriched contact with data from website. Confidence: ${(confidenceScore * 100).toFixed(0)}%`, metadata: { extractedData, confidenceScore, url: targetUrl, autoDetected }, timestamp: new Date().toISOString() });
    } catch (historyErr) {
      console.error('[enrichment] History insert error (non-critical):', historyErr);
    }

    return res.status(200).json({ success: true, extracted: extractedData, contact: data, enrichmentMetadata: { url: targetUrl, source: 'web_scrape', confidence: confidenceScore, autoDetected } });
  } catch (err: any) {
    console.error('[enrichment] enrichLead error:', err);
    return res.status(500).json({ success: false, error: `Server error: ${err.message || 'Unknown error'}`, details: err.toString() });
  }
}
